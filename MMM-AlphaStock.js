...
Module.register("MMM-AlphaStock", {
  ...
  fetchStockPrice: async function() {
    ...
    const response = await fetch(`https://www.alphavantage.co/query?function=${type}&symbol=${symbol}&apikey=${apikey}`)
            ...
            options: {
                ...
                plugins: {
					datalabels: {
						...
						borderRadius: 4,
						color: 'black',
                        display: (context) => this.config.showLastPointLabel && context.dataIndex === chartData.y.length - 1,
